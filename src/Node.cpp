...
void Node::handleHello(cMessage *msg){
    ...
    if (senderIndex == 4) { // magic no for demo. it's malcs.
...
void Node::RREQ() {
    ...
    rreq->addPar("NEIGHBORS_STORAGE").setStringValue((Util::vectorToString(komsu) + ";" + neighborsOfMyNeighbors).c_str());
...
void Node::handleRREQ(AODVRREQ *rreq) {
    ...
    if (rreq != nullptr) {
        ...
        if (it != komsu.end()) {
            ...
            neighborsOfMyNeighbors = senderNode + ":" + senderStorage;
            if (nodeIndex == hedef) {
                neighborsOfNodesOnTheRoute.push_back(neighborsOfMyNeighbors);
                EV << "Neighbors Of Nodes Which On The Route : " <<  neighborsOfMyNeighbors << endl;
            }
            ...
            if (nodeIndex == hedef) {
                ...
                EV << "BEN HEDEF ::: BURADAN RREQ ALDIM :::" <<  senderNode << endl;
